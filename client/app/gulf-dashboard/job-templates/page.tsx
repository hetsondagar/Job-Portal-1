  ...
  const handleCreateJobFromTemplate = async (templateId: string) => {
    try {
      ...
      if (response.success) {
        ...
        window.location.href = `/gulf-dashboard/post-job?draft=${job.id}`
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            ...
            <p className="text-slate-600">Create and manage reusable job posting templates for Gulf region</p>
          </div>
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a reusable template for Gulf job postings
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {template.tags.length > 3 && (
                      <Badge variant="outline" className="text-xs">+{template.tags.length - 3} more</Badge>
                    )}
                  </div>
                  <div className="space-y-3">
                    <Button className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700" onClick={() => handleCreateJobFromTemplate(template.id)}>
                      <Plus className="w-4 h-4 mr-2" />
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex space-x-1">
                        {template.createdBy === user?.id && (
                          <Button size="sm" variant="outline" onClick={() => handleEditTemplate(template)} title="Edit Template">
                            <Edit className="w-4 h-4" />
                          </Button>
                        )}
                        <Button size="sm" variant="outline" onClick={() => handleUseTemplate(template.id)} title="Copy Template">
                          <Copy className="w-4 h-4" />
                        </Button>
                        {template.createdBy === user?.id && (
                          <Button size="sm" variant="outline" onClick={() => handleTogglePublic(template.id)} title={template.isPublic ? "Make Private" : "Make Public"}>
                            {template.isPublic ? <Lock className="w-4 h-4" /> : <Globe className="w-4 h-4" />}
                          </Button>
                        )}
                      </div>
                      {template.createdBy === user?.id && (
                        <Button size="sm" variant="destructive" onClick={() => handleDeleteTemplate(template.id)} title="Delete Template">
        ...
        {filteredTemplates.length === 0 && !loading && (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-slate-600 mb-4">{searchQuery || selectedCategory !== 'all' ? 'Try adjusting your search criteria' : 'Create your first job template to get started'}</p>
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Input id="name" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g., Senior Software Engineer" required />
        </div>
        <div>
      ...
      <div>
        ...
        <Textarea id="description" value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Describe what this template is for..." rows={3} />
      </div>
      <div>
        <Label htmlFor="tags">Tags</Label>
        <div className="flex space-x-2 mb-2">
          <Input value={currentTag} onChange={(e) => setCurrentTag(e.target.value)} placeholder="Add a tag..." onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddTag())} />
...
function EditTemplateForm({ template, onSubmit, loading }: { template: JobTemplate, onSubmit: (data: any) => void, loading: boolean }) {
  const [formData, setFormData] = useState({
    name: template.name,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end space-x-3">
        <Button type="button" variant="outline">Cancel</Button>
        <Button type="submit" disabled={loading}>{loading ? 'Updating...' : 'Update Template'}</Button>
      </div>
    </form>
  )
}
