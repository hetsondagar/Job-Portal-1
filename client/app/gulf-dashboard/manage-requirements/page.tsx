...
export default function GulfManageRequirementsPage() {
  ...
  // Fetch requirements from API - filter by Gulf region
  useEffect(() => {
    const fetchRequirements = async () => {
      try {
        ...
        console.log('🔍 Gulf Frontend: Starting to fetch Gulf requirements...')
        console.log('🔍 Gulf Frontend: Checking authentication...')
        ...
        console.log('🔍 Gulf Frontend: Token exists:', !!token)
        console.log('🔍 Gulf Frontend: User exists:', !!user)
        console.log('🔍 Gulf Frontend: Company exists:', !!company)
        
        if (!token || !user) {
          console.log('❌ Gulf Frontend: User not authenticated, redirecting to login')
        ...
        console.log('🔍 Gulf Frontend: Refreshing user data from server...')
        try {
          const userResponse = await apiService.getCurrentUser()
          if (userResponse.success && userResponse.data?.user) {
            console.log('✅ Gulf Frontend: Successfully refreshed user data')
            localStorage.setItem('user', JSON.stringify(userResponse.data.user))
            const refreshedUser = userResponse.data.user
            console.log('🔍 Gulf Frontend: Refreshed user data:', {
              id: refreshedUser.id,
            ...
            if (refreshedUser.userType !== 'employer' && refreshedUser.userType !== 'admin') {
              console.log('❌ Gulf Frontend: User is not an employer')
              console.log('❌ Gulf Frontend: Expected "employer", got:', refreshedUser.userType)
              setError('Access denied. Only employers and admins can view requirements.')
              return
            }
          } else {
            console.log('❌ Gulf Frontend: Failed to refresh user data')
            // Fall back to localStorage data
            const userData = JSON.parse(user)
            console.log('🔍 Gulf Frontend: Using localStorage user data:', userData)
            
            // Check if user is an employer or admin
            if (userData.userType !== 'employer' && userData.userType !== 'admin') {
              console.log('❌ Gulf Frontend: User is not an employer')
              console.log('❌ Gulf Frontend: Expected "employer", got:', userData.userType)
              setError('Access denied. Only employers and admins can view requirements.')
              return
            }
          }
        } catch (userError) {
          console.log('❌ Gulf Frontend: Error refreshing user data, using localStorage:', userError)
          // Fall back to localStorage data
          const userData = JSON.parse(user)
        ...
        console.log('🔍 Gulf Frontend: API response:', response)
        
        if (response.success && response.data) {
          console.log('✅ Gulf Frontend: Successfully fetched requirements:', response.data.length)
          // Filter requirements by Gulf region (from metadata)
          const gulfRequirements = response.data.filter((req: Requirement) => {
            const metadata = (req as any).metadata || {}
            const region = metadata.region || 'india'
            return region === 'gulf'
          })
          console.log('✅ Gulf Frontend: Filtered Gulf requirements:', gulfRequirements.length)
          setRequirements(gulfRequirements)
          
          // Fetch stats for each requirement
          const statsPromises = gulfRequirements.map(async (requirement: Requirement) => {
            try {
          ...
          console.log('✅ Gulf Frontend: Successfully fetched requirement stats:', statsMap)
        } else {
          console.error('❌ Gulf Frontend: Failed to fetch requirements:', response.message)
        ...
        console.error('❌ Gulf Frontend: Error fetching requirements:', error)
  ...
  const refreshStats = useCallback(() => {
    console.log('🔄 Refreshing Gulf requirement stats...');
  ...
  console.log('🔍 Gulf Frontend: Filtered requirements count:', filteredRequirements.length)
  console.log('🔍 Gulf Frontend: Total requirements count:', requirements.length)
  ...
  const handleEditRequirement = (id: string) => {
    router.push(`/gulf-dashboard/manage-requirements/${id}/edit`)
    toast({
    ...
    <EmployerAuthGuard>
      <GulfEmployerAuthGuard>
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/40 to-teal-50/40 dark:from-gray-900 dark:via-gray-800/50 dark:to-gray-900 relative overflow-auto">
          ...
          {/* Background Effects - Gulf theme (emerald/teal) */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {/* Base emerald gradient overlay to ensure visible background */}
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-gradient-to-br from-teal-300/10 to-emerald-300/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
            {/* Wide translucent emerald gradient strip */}
            <div className="absolute top-1/3 left-0 right-0 h-24 bg-gradient-to-r from-emerald-400/20 via-teal-400/20 to-cyan-400/20"></div>
          </div>
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h1 className="text-3xl font-bold text-slate-900 mb-2">My Gulf Requirements</h1>
                <p className="text-slate-600">Manage your Gulf region requirements</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-1">
                <Card className="rounded-3xl bg-white/50 backdrop-blur-2xl border-white/40 shadow-[0_8px_28px_rgba(16,185,129,0.08)] hover:shadow-[0_18px_60px_rgba(16,185,129,0.16)] sticky top-24">
                  <CardHeader>
                      ...
                      {loading ? (
                        ...
                        filteredRequirements.map((requirement, index) => (
                          <motion.div
                            ...
                            className="border border-slate-200 rounded-lg p-6 hover:bg-slate-50 transition-colors mb-4"
                          >
                            <div className="flex items-start justify-between">
                              ...
                              <div className="flex items-center space-x-6">
                                <div className="text-center">
                                  <Link href={`/gulf-dashboard/manage-requirements/${requirement.id}/candidates`}>
                                    <div className="text-2xl font-bold text-slate-900 hover:text-emerald-600 cursor-pointer transition-colors">
                                ...
                                <div className="text-center">
                                  <div className="text-2xl font-bold text-emerald-600">{getAccessedCount(requirement)}</div>
                          ...
                          <p className="text-slate-600 mb-4">
                            {searchQuery 
                              ...
                              : requirements.length === 0 
                                ? "You haven't created any Gulf requirements yet. Create your first requirement to get started."
