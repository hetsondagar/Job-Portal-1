...
class GulfCompanyErrorBoundary extends React.Component<
...
function GulfCompanyDetailPage() {
      ...
      if (!isValidUuid) {
        ...
        try {
          ...
          if (filterParams.experience && filterParams.experience !== 'all') {
            params.experienceLevel = filterParams.experience
          }
          if (filterParams.salary && filterParams.salary !== 'all') {
            // Map salary filter to salaryMin/salaryMax
            if (filterParams.salary === 'low') {
              params.salaryMax = 5000
            } else if (filterParams.salary === 'medium') {
              params.salaryMin = 5000
              params.salaryMax = 15000
            } else if (filterParams.salary === 'high') {
              params.salaryMin = 15000
            }
          }
          ...
          if (response.success && response.data) {
            // Handle both response formats: { jobs: [], pagination: {} } or just []
          ...
          console.log('Gulf jobs API failed:', error?.message || error)
          // Try fallback: use regular companies endpoint with region filter
          try {
            const queryParams = new URLSearchParams()
            queryParams.append('region', 'gulf')
            ...
            const url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/companies/${companyId}/jobs?${queryParams.toString()}`
            ...
            console.log('Fallback endpoint also failed:', altError)
          }
          setCompanyJobs([])
          setFilteredJobs([])
  ...
  const fetchCompanyPhotos = useCallback(async () => {
    try {
      ...
      const resp = await fetch(`${base}/gulf-companies/${companyId}/photos`)
      if (resp.ok) {
  ...
  const handlePhotoDelete = useCallback(async (photoId: string) => {
    ...
    try {
      ...
      const base = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'
      const res = await fetch(`${base}/gulf-companies/photos/${photoId}`, {
        method: 'DELETE',
  ...
  const handleShare = (platform: string) => {
    const companyUrl = `${window.location.origin}/gulf-companies/${company.id}`
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
              <TabsList className={`grid w-full ${company?.whyJoinUs || company?.why_join_us ? 'grid-cols-3' : 'grid-cols-2'} bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg rounded-xl`}>
                ...
                <TabsTrigger value="why" className="data-[state=active]:bg-green-600 data-[state=active]:text-white">
                  Why Join Us
                  </TabsTrigger>
                )}
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {departments.map((dept, index) => (
                        <Link key={dept.name + index} href={`/gulf-companies/${companyId}/departments/${encodeURIComponent(dept.name)}`}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className={filters.location !== 'all' ? "bg-green-50 border-green-200 text-green-600" : ""}
                  >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className={filters.experience !== 'all' ? "bg-green-50 border-green-200 text-green-600" : ""}
                  >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className={filters.salary !== 'all' ? "bg-green-50 border-green-200 text-green-600" : ""}
                  >
                    Salary (3)
              </Button>
                </DropdownMenuTrigger>
                                ...
                                <Button
                                  ...
                                  className={`h-10 px-6 ${
                                    hasAppliedToJob(job.id)
                                      ...
                                      : `bg-gradient-to-r from-green-600 to-emerald-600 hover:shadow-lg transition-all duration-300`
                                  }`}
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700">
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                {user && user.userType === 'jobseeker' ? (
                                  <Button
                                    ...
                                    className={`${
                                      hasAppliedToJob(job.id)
                                        ...
                                        : job.status === 'expired'
                                          ? 'bg-slate-300 cursor-not-allowed'
                                          : 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-blue-700 hover:to-indigo-700'
                                    } text-white`}
                                  ...
                                  <Button
                                    size="sm"
                                    className={`${job.status === 'expired' ? 'bg-slate-300 cursor-not-allowed' : 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-blue-700 hover:to-indigo-700'} text-white`}
      ...
      <Dialog open={showAuthDialog} onOpenChange={setShowAuthDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col space-y-3 mt-6">
            <Link href="/register">
              <Button className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-blue-700 hover:to-indigo-700">
                Register Now
              </Button>
            </Link>
            <Link href="/login">
...
export default dynamic(() => Promise.resolve(GulfCompanyDetailPage), { 
  ssr: false,
  loading: () => (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/30 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      <div className="pt-20 pb-8">
