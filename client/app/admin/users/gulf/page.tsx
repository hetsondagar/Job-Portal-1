...
export default function GulfUsersPage() {
  ...
  const loadGulfUsers = async () => {
    try {
      setLoading(true)
      const response = await apiService.getUsersByRegion('gulf', {
        page: currentPage,
        limit: 20,
        search: searchTerm,
      ...
      if (response.success) {
        ...
        toast.error('Failed to load Gulf users')
      }
    } catch (error) {
      console.error('Failed to load Gulf users:', error)
      toast.error('Failed to load Gulf users')
    } finally {
      setLoading(false)
    }
  ...
  const exportGulfUsers = async () => {
    try {
      ...
      if (response.success) {
        ...
        a.href = url
        a.download = `gulf-users-export-${new Date().toISOString().split('T')[0]}.csv`
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        window.URL.revokeObjectURL(url)
        
        toast.success('Gulf users exported successfully')
      } else {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Manage users from Gulf region</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <CardContent>
            {users.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-400">No Gulf users found</p>
              </div>
            ) : (
        ...
        <Dialog open={showUserDialog} onOpenChange={setShowUserDialog}>
          <DialogContent className="max-w-2xl bg-slate-800 border-white/20 text-white">
            <DialogHeader>
              <DialogTitle>Gulf User Details</DialogTitle>
              <DialogDescription>
