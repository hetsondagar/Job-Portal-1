...
export default function GulfJobsPage() {
  ...
  const loadGulfJobs = async () => {
    try {
      setLoading(true)
      const response = await apiService.getJobsByRegion('gulf', {
        page: currentPage,
        limit: 20,
        search: searchTerm,
      ...
      console.error('Failed to load Gulf jobs:', error)
      toast.error('Failed to load Gulf jobs')
    } finally {
      setLoading(false)
    }
  ...
  const exportGulfJobs = async () => {
    try {
      ...
      if (response.success) {
        ...
        a.href = url
        a.download = `gulf-jobs-export-${new Date().toISOString().split('T')[0]}.csv`
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        window.URL.revokeObjectURL(url)
        
        toast.success('Gulf jobs exported successfully')
      } else {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Manage jobs from Gulf region</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <CardContent>
            {jobs.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-400">No Gulf jobs found</p>
              </div>
            ) : (
        ...
        <Dialog open={showJobDialog} onOpenChange={setShowJobDialog}>
          <DialogContent className="max-w-4xl bg-slate-800 border-white/20 text-white max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Gulf Job Details</DialogTitle>
              <DialogDescription>
