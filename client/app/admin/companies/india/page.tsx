...
export default function IndiaCompaniesPage() {
  ...
  const loadIndiaCompanies = async () => {
    try {
      setLoading(true)
      const response = await apiService.getCompaniesByRegion('india', {
        page: currentPage,
        limit: 20,
        search: searchTerm,
      ...
      if (response.success) {
        ...
        toast.error('Failed to load India companies')
      }
    } catch (error) {
      console.error('Failed to load India companies:', error)
      toast.error('Failed to load India companies')
    } finally {
      setLoading(false)
    }
  ...
  const exportIndiaCompanies = async () => {
    try {
      ...
      if (response.success) {
        ...
        a.href = url
        a.download = `india-companies-export-${new Date().toISOString().split('T')[0]}.csv`
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        window.URL.revokeObjectURL(url)
        
        toast.success('India companies exported successfully')
      } else {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Manage companies from India region</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-2">
                <div className="relative">
                  ...
                  <Input
                    placeholder="Search India companies..."
                    value={searchTerm}
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span className="flex items-center">
                ...
                India Companies ({companies.length})
              </span>
          ...
          <CardContent>
            {companies.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-400">No India companies found</p>
              </div>
            ) : (
        ...
        <Dialog open={showCompanyDialog} onOpenChange={setShowCompanyDialog}>
          <DialogContent className="max-w-4xl bg-slate-800 border-white/20 text-white max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>India Company Details</DialogTitle>
              <DialogDescription>
            ...
            {selectedCompany && (
              <div className="space-y-6">
                <div className="flex items-start space-x-4">
                  <div className="w-20 h-20 bg-gradient-to-br from-orange-500 to-red-600 rounded-lg flex items-center justify-center">
